...
$(function () {
  // License is verified by popup.js before opening this panel; run directly
  init();
});
...
// =================================================================
// CWP LICENSE BAR — show deactivate button in panel footer
// =================================================================
(function cwpLicenseBar() {
  const STORAGE_KEY  = "cwp_license";
  const CHECK_ALARM  = "cwp_license_recheck";
  const bar          = document.getElementById("cwp_license_bar");
  const hintEl       = document.getElementById("cwp_license_hint");
  const deactivateBtn= document.getElementById("cwp_deactivate_btn");

  if (!bar || !deactivateBtn) return;

  // Load license and display the bar if premium
  chrome.storage.local.get(STORAGE_KEY, function(result) {
    const lic = result[STORAGE_KEY];
    if (lic && lic.premium && lic.key) {
      const hint = lic.key.split("-")[0] + "-\u2022\u2022\u2022\u2022-\u2022\u2022\u2022\u2022-\u2022\u2022\u2022\u2022";
      const planLabel = lic.lifetime ? "Lifetime" : (lic.plan || "Premium");
      hintEl.textContent = "\uD83D\uDD11 " + planLabel + "  \u00B7  " + hint;
  ...
  deactivateBtn.addEventListener("click", function() {
    if (!confirm("Deactivate your CWP license on this device?\nYou can re-activate at any time in the popup.")) return;
    chrome.storage.local.remove(STORAGE_KEY, function() {
      ...
      msg.textContent = "\u2705 License deactivated. Re-activate via the extension popup.";
      msg.style.cssText = "position:fixed;bottom:20px;left:50%;transform:translateX(-50%);background:#333;color:#fff;padding:10px 18px;border-radius:8px;font-size:12px;z-index:9999;";
